...
O manifesto de referência está em `docs/files-system-app.compose.yml`: a porta é publicada somente em `127.0.0.1:8081`, o usuário só recebe `/DATA:/srv` e a configuração fica em `/var/lib/labsos/system-apps/files`. O System App está provisionado no Debian e saudável. Em desenvolvimento, o frontend usa `/file-manager/`, encaminhado pelo Vite para `localhost:18081` através do túnel SSH.
