...
`labsd` é a fronteira tipada aprovada: roda como root, expõe somente `/run/labsos/labsd.sock` para o `agent` e aceita operações fixas para manifests conhecidos. O primeiro manifest executável é `specs/jellyfin.compose.yml`, com porta apenas em loopback e mounts limitados a `/DATA/Apps/jellyfin` e `/DATA/Media:ro`.
